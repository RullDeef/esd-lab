...
std::list<int> GraphSearch::DoDepthFirstSearch() {
  ...
  while (!m_foundSolution && !m_noSolution) {
    ...
    /* вызываем метод поиска потомков */
...
std::list<int> GraphSearch::DoBreadthFirstSearch() {
  ...
  - пока флаги истинны, выполняем:
      ...
      - проверяем, если число потомков не равно нулю, пишем эту вершину в список
  закрытых вершин
      - проверяем, если флаг решения 0 - выходим и выводим список закрытых
  вершин
      - иначе, если число потомков 0 и список открытых вершин пуст - то
  сбрасываем в 0 флаг "нет решения" и выходим из цикла
  */
  ShowState(true);
  while (!m_foundSolution && !m_noSolution) {
  ...
  while (m_closedNodes.size() > 0 && lastNode != m_closedNodes.front()) {
...
int GraphSearch::DescendantsBFS(int node) {
  ...
  - пока не конец базы правил:
      ...
      - иначе (находим инцидентные ребра, начальная вершина которых совпадает с
  образцом)
      - проверяем, что образец совпадает с начальной вершиной ребра, метка ребра
  равна нулю (оно еще не выбрано)
      - если конечная вершина равна цели, то флаг решения в 0, и можно выйти из
  цикла.
      - иначе - находим индидентное ребро, то есть те же условия, что были,
  только без цели
